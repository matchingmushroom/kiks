...
const navItems: NavItem[] = [
  ...
  { label: "POS", href: "/pos", icon: <Zap className="h-4 w-4" />, permission: "manage_sales" },
  { label: "Sales", href: "/sales", icon: <ShoppingCart className="h-4 w-4" />, permission: "manage_sales" },
  { label: "Invoices", href: "/invoices", icon: <FileText className="h-4 w-4" />, permission: "manage_invoices" },
  { label: "Orders", href: "/orders", icon: <Truck className="h-4 w-4" />, permission: "manage_orders" },
  { label: "Coupons", href: "/coupons", icon: <Tags className="h-4 w-4" />, permission: "manage_coupons" },
  { label: "Offers", href: "/offers", icon: <Sparkles className="h-4 w-4" />, permission: "manage_offers" },
  { label: "Testimonials", href: "/testimonials", icon: <MessageSquare className="h-4 w-4" />, permission: "manage_homepage" },
  { label: "Debtors", href: "/debtors", icon: <Users className="h-4 w-4" />, permission: "manage_debtors" },
  { divider: true, section: "LOYALTY" },
  { label: "Loyalty", href: "/loyalty", icon: <Award className="h-4 w-4" />, permission: "manage_sales" },
  { divider: true, section: "INVENTORY" },
  { label: "Morning Dashboard", href: "/morning", icon: <BarChart3 className="h-4 w-4" />, permission: "view_morning_dashboard" },
  { label: "Products", href: "/products", icon: <Package className="h-4 w-4" />, permission: "manage_products", liteModeHide: true },
  { label: "Categories", href: "/categories", icon: <Tags className="h-4 w-4" />, permission: "manage_categories" },
  { label: "Inventory", href: "/inventory", icon: <BarChart3 className="h-4 w-4" />, permission: "manage_inventory", liteModeHide: true },
  { label: "Reconciliation", href: "/reconciliation", icon: <ClipboardList className="h-4 w-4" />, permission: "manage_inventory", liteModeHide: true },
  { label: "Purchases", href: "/purchases", icon: <DollarSign className="h-4 w-4" />, permission: "manage_purchases" },
  { label: "Suppliers", href: "/suppliers", icon: <Truck className="h-4 w-4" />, permission: "manage_suppliers", liteModeHide: true },
  { label: "Creditors", href: "/creditors", icon: <Users className="h-4 w-4" />, permission: "manage_creditors" },
  { divider: true, section: "FINANCE" },
  { label: "Expenses", href: "/expenses", icon: <CreditCard className="h-4 w-4" />, permission: "manage_expenses" },
  { label: "Reports", href: "/reports", icon: <FileText className="h-4 w-4" />, permission: "view_reports" },
  { label: "Finance", href: "/finance", icon: <Wallet className="h-4 w-4" />, permission: "view_reports" },
  { label: "Accounting", href: "/accounting", icon: <FileText className="h-4 w-4" />, permission: "view_reports" },
  { divider: true, section: "PEOPLE" },
  { label: "Customers", href: "/customers", icon: <Users className="h-4 w-4" />, permission: "manage_customers" },
  { label: "Staff", href: "/staff", icon: <Shield className="h-4 w-4" />, permission: "manage_staff" },
  { divider: true, section: "SYSTEM" },
  { label: "Homepage", href: "/homepage", icon: <Home className="h-4 w-4" />, permission: "manage_homepage" },
  { label: "Access Control", href: "/access-control", icon: <Shield className="h-4 w-4" />, permission: "manage_access_control" },
  { label: "Backup", href: "/backup", icon: <Receipt className="h-4 w-4" />, permission: "manage_backup" },
  { label: "Settings", href: "/settings", icon: <Settings className="h-4 w-4" />, permission: "manage_settings" },
  { label: "Setup", href: "/setup", icon: <Rocket className="h-4 w-4" />, permission: "manage_settings" },
];

// Icon map for bottom nav (uses larger h-5 w-5 icons)
const bottomNavIconMap: Record<string, ReactNode> = {
  "/": <LayoutDashboard className="h-5 w-5" />,
  "/pos": <Zap className="h-5 w-5" />,
  "/sales": <ShoppingCart className="h-5 w-5" />,
  "/invoices": <FileText className="h-5 w-5" />,
  "/orders": <Truck className="h-5 w-5" />,
  "/coupons": <Tags className="h-5 w-5" />,
  "/offers": <Sparkles className="h-5 w-5" />,
  "/testimonials": <MessageSquare className="h-5 w-5" />,
  "/debtors": <Users className="h-5 w-5" />,
  "/morning": <BarChart3 className="h-5 w-5" />,
  "/products": <Package className="h-5 w-5" />,
  "/categories": <Tags className="h-5 w-5" />,
  "/inventory": <BarChart3 className="h-5 w-5" />,
  "/reconciliation": <ClipboardList className="h-5 w-5" />,
  "/purchases": <DollarSign className="h-5 w-5" />,
  "/suppliers": <Truck className="h-5 w-5" />,
  "/creditors": <Users className="h-5 w-5" />,
  "/expenses": <CreditCard className="h-5 w-5" />,
  ...
  "/staff": <Shield className="h-5 w-5" />,
  "/homepage": <Home className="h-5 w-5" />,
  "/access-control": <Shield className="h-5 w-5" />,
  "/backup": <Receipt className="h-5 w-5" />,
  "/settings": <Settings className="h-5 w-5" />,
  "/setup": <Rocket className="h-5 w-5" />,
};

// Default bottom nav items (hrefs)
const defaultBottomNavHrefs = ["/pos", "/sales", "/", "/products", "/purchases"];
...
function BottomNav({ profile, pathname, settings }: { profile: any; pathname: string; settings?: any }) {
  ...
  if (isLite) {
    items = [
      ...
      { href: "/expenses", label: "Expenses", icon: bottomNavIconMap["/expenses"] || <CreditCard className="h-5 w-5" /> },
      { href: "/pos", label: "POS", icon: bottomNavIconMap["/pos"] || <Zap className="h-5 w-5" /> },
      { href: "/", label: "Dashboard", icon: bottomNavIconMap["/"] || <LayoutDashboard className="h-5 w-5" /> },
      { href: "/debtors", label: "Debtors", icon: bottomNavIconMap["/debtors"] || <Users className="h-5 w-5" /> },
    ];
  } else {
    items = hrefs
      .map((href) => {
  ...
  if (!permitted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#FFFDFB] gap-5 px-4">
        ...
        <Link href="/" className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground font-medium rounded-xl text-sm hover:bg-primary-hover transition-all duration-200 shadow-[0_10px_30px_rgba(107,0,46,0.20)]">
